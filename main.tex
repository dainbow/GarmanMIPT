...
\begin{note}
	...
	В доказательствах некоторых теорем этого конспекта используется интересный трюк: если у нас есть цепочка равенств $a = b$, то мы с лёгкостью сможем продолжить её, написав $a = b = \frac{a + b}{2}$. Если вы понимаете, что в доказательстве теоремы с интегралами происходит какая-то дичь, то вспоминайте этот трюк! 
\end{note}
...
\begin{proof}
	\begin{align*}
		\int_I f(x) \cos(\lambda x) d\mu(x) \stackrel{x = t + \frac{\pi}{\lambda}}{=} - \int_{I - \frac{\pi}{\lambda}} f\left( t + \frac{\pi}{\lambda} \right) \cos(\lambda t) d \mu(t) \stackrel{\text{Трюк}}{=} \\
...
\begin{proof}
	...
	\begin{align*}
		a_n(\chi) = \frac{1}{\pi} \int_{-\pi}^\pi \chi(t)\cos(nt)d\mu(t) \stackrel{t = u + \frac{\pi}{n}}{=} \frac{-1}{\pi}\int_{-\pi}^\pi \chi\left(u + \frac{\pi}{n}\right)\cos(nu)d\mu(u) \stackrel{\text{Трюк}}{=} \\
...
\begin{proof}
	Рассмотрим $a_n$:
	\begin{align*}
		a_n = \frac{1}{\pi}\int_{-\pi}^\pi f(x)\cos(nx)dx \stackrel{\text{Трюк}}{=} \frac{-1}{2\pi}\int_{-\pi}^\pi \left[f(x + \frac{\pi}{n}) - f(x)\right]\cos(nx)dx = \cdots = \\
...
\begin{proof}
	Заметим, что 
	\[\tilde{\hat{f}}(u) = \frac{1}{2\pi}\int_\mathbb{R}\int_\mathbb{R}f(t)e^{-i\lambda t}d\mu(t)e^{iu\lambda}d\mu(\lambda) = \frac{1}{2\pi}\int_\mathbb{R}\int_\mathbb{R} f(t)e^{-i\lambda(u - t)}d\mu(t)d\mu(\lambda)\]
	Далее, из суммируемости $f$ следует, что интеграл
	\[\int_\mathbb{R}f(t)\sin(\lambda(t - u))d\mu(t)\]
	существует и является нечётной функцией относительно $\lambda$. Поэтому
	\[\frac{1}{2\pi}\int_\mathbb{R}\int_\mathbb{R} f(t) \sin(\lambda(t - u))d\mu(t) d\mu(\lambda) = 0\]
	Вычтем из первой формуле доказательства этот двойной интеграл, умноженный на $-i$, ничего не изменится, так как он равен $0$, но в итоге получим, что
	\[\tilde{\hat{f}}(u) = \frac{1}{2\pi} \int_\mathbb{R}\int_\mathbb{R} f(t) \cos(\lambda(t - u))d\mu(t)d\mu(\lambda)\]
	а это интеграл Фурье, тогда утверждение становится очевидным, благодаря одному из следствий признака Дини (\ref{DINI_FURRY}).
\end{proof}
